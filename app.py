...
st.set_page_config(page_title="HidroSed SedGran v3.7.7 Secciones Compuestas", page_icon="🌊", layout="wide")
...
st.sidebar.title("HidroSed SedGran v3.7.6C")
st.sidebar.caption("Centro de control hidráulico-hidrológico · QA · 3D · trazabilidad · hotfix imports isoyetas/normativa")
for k, label in [
...
st.markdown(
    """
<div class='hs-hero'>
  <h1>🌊 HidroSed SedGran v3.7.7 · Secciones compuestas · Hidrología · Sedimentos</h1>
  <p>Plataforma hidráulica-hidrológica avanzada para cuencas y cauces: DEM OpenTopography, delimitación, curvas, secciones reales + normativa o trapezoidales, hidrología normativa, hidráulica 1D tipo HEC‑RAS mejorada, rugosidad avanzada, granulometría georreferenciada, sedimentos, socavación, QA, incertidumbre y visualización 3D.</p>
        ...
        if st.button("Generar lámina cartográfica PNG", type="primary"):
            try:
                png = make_cartographic_sheet(
                    ...
                    axis_line=_axis_line_coords(st.session_state.get("axis_line")),
...
"""
    ...
    with audit_tabs[6]:
        ...
        md = technical_markdown_report(
            "Informe de Auditoría General HidroSed SedGran v3.7.6C",
        ...
        try:
            docxb = docx_report_bytes("Informe de Auditoría General HidroSed SedGran v3.7.6C", st.session_state.get("aud_external_audit_df", pd.DataFrame()), st.session_state.get("aud_score_df", pd.DataFrame()), st.session_state.get("aud_adoption_df", pd.DataFrame()))
        ...
        try:
            pdfb = pdf_report_bytes("Informe de Auditoría General HidroSed SedGran v3.7.6C", st.session_state.get("aud_external_audit_df", pd.DataFrame()), st.session_state.get("aud_score_df", pd.DataFrame()), st.session_state.get("aud_adoption_df", pd.DataFrame()))
